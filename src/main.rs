...
fn main() -> std::io::Result<()> {
    ...
    // 5. Native Responsive Theme CSS (Moved back inline)
    let css_styles = r#":root {
    --bg-color: #ffffff;
    --text-color: #24292f;
    --link-color: #0969da;
    --border-color: #d0d7de;
...
"#;
    ...
    // 6. Assemble HTML template mapping styles straight into a structural <style> element
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/prism/1.29.0/plugins/autoloader/prism-autoloader.min.js"></script>
    ...
    // 7. Write everything to a single autonomous HTML target file
    let mut html_file = File::create(&output_path)?;
    html_file.write_all(full_html.as_bytes())?;
    println!("🎉 Success! Standalone file generated at: {}", output_path);
